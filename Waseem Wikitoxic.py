...
dataset_name1 = f"./all_data_Waseem.pickle"
dataset_name2 = f"./all_data_Wikitoxic.pickle"
output_filename = f"Waseem Wikitoxic.json"

data1 = pickle.load(open(dataset_name1, "rb"))
data2 = pickle.load(open(dataset_name2, "rb"))
...
print(data1["class_names"])
print(data2["class_names"])
...
X_train = []
for text in tqdm(data1["text_train"]):
...
X_validate = []
for text in tqdm(data1["text_validate"]):
    tokens = tokenizer(text)
    vector = []
    for token in tokens:
        if token.text.lower() in vocab:
...
X_test = []
for text in tqdm(data2["text_test"]):
    tokens = tokenizer(text)
    vector = []
    for token in tokens:
        if token.text.lower() in vocab:
...
ans = MaskedDense(len(class_names1), activation='softmax')(concatenated)

model = Model(text_input, ans)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
model.summary()

y_train_onehot, y_validate_onehot = to_categorical(data1['y_train']), to_categorical(data1['y_validate'])
...
y_true, y_predict = np.array(y_test2), np.array(prediction_test)
for idx, cname in enumerate(class_names2):
...
for feature_idx in range(30):
    ...
    scores = {}
    for idx, cn in enumerate(class_names2):
